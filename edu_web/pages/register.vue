    ...
    <div class="sign-up-container">
      <el-form ref="userForm" :model="registerVo">
        ...
        <el-form-item class="input-prepend restyle no-radius" prop="email"
                      :rules="[{ required: true, message: '请输入邮箱', trigger: 'blur' },{required: true, trigger: 'blur'}]">  
                      <!-- validator: checkPhone,  -->
          <div>
            <el-input type="text" placeholder="邮箱" v-model="registerVo.email"/>
...
export default {
  ...
  methods: {
    getCodeFun(){//发送验证码
      ...
      registerApi.sendCode(this.registerVo.email).then(res=>{
        this.sending = false;
        this.timeDown();//调用倒计时方法
      })
    },
    submitRegister(){ //注册
    console.log("注册")
      registerApi.registerUser(this.registerVo).then(res=>{
